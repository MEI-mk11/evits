...
def run(rank, n_gpus, hps):
    ...
    optim_d = RAdam(net_d.parameters(), 1e-4)
    optim_p = RAdam(net_p.parameters(), 1e-4) if use_dur_dis else None
...
def train_and_evaluate(rank, epoch, hps, nets, optims, scaler, loaders, logger, writers):
    ...
    for batch_idx, (x, x_lengths, spec, spec_lengths, y, y_lengths, emo, speakers) in enumerate(train_loader):
        ...
        with autocast(enabled=hps.train.fp16_run):
            ...
            y_mel = commons.slice_segments(mel, ids_slice[:1], hps.train.segment_size // hps.data.hop_length)
